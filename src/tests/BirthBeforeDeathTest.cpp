...
void BirthBeforeDeathTest::testNoBirthDateSpecified(){
	CPPUNIT_ASSERT_MESSAGE("no birth date, should be valid",(testReader->checkValidBirthBeforeDeath(testReader->Individuals[2])));

}
void BirthBeforeDeathTest::testNoDeathDateSpecified(){
	CPPUNIT_ASSERT_MESSAGE("no death date, should be valid",(testReader->checkValidBirthBeforeDeath(testReader->Individuals[3])));
